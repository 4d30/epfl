...
object Anagrams extends AnagramsInterface:
   ...
   */
  def wordOccurrences(w: Word): Occurrences = Option(w) match {
    case None => Nil
    case Some(cs) => {
      for
        (c, l) <- cs.toLowerCase.toList.groupBy(c => c).toList.sorted(Ordering[Char].on(x => x._1))
      yield
        (c, l.length)
    }
  }
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = Option(s) match {
    case None => Nil
    case Some(ws) => wordOccurrences(ws.mkString(""))
  }
  ...
  def wordAnagrams(word: Word): List[Word] = 
    dictionaryByOccurrences(wordOccurrences(word)) match {
      case Nil => List(word)
      case ws => ws
    }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def combinationsHelper(occurrences: Occurrences): List[Occurrences] = occurrences match {
      ...
      case (c, i) :: os => {
        for
          I <- 0 to i 
          tail <- combinations(os)
        yield (c, I) :: tail
      }.toList
    }
    val combos = combinationsHelper(occurrences).toSet
    combos.map(x => x.filter(_._2 != 0)).toList
  }
